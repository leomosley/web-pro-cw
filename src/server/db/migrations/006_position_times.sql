...
CREATE TABLE IF NOT EXISTS race (
  ...
  checked_in_percentage REAL NOT NULL DEFAULT 0,
...
CREATE TRIGGER IF NOT EXISTS update_checked_in_percentage_on_update
AFTER UPDATE ON race_participant
FOR EACH ROW
BEGIN
  UPDATE race
  SET checked_in_percentage = (
    SELECT 
      CASE 
        WHEN COUNT(*) = 0 THEN 0.0
        ELSE (COUNT(CASE WHEN checked_in THEN 1 END) * 100.0) / COUNT(*)
      END
...
CREATE TRIGGER IF NOT EXISTS update_checked_in_percentage_on_delete
AFTER DELETE ON race_participant
FOR EACH ROW
BEGIN
  UPDATE race
  SET checked_in_percentage = (
...
CREATE TRIGGER IF NOT EXISTS update_checked_in_percentage_on_insert
AFTER INSERT ON race_participant
FOR EACH ROW
BEGIN
  UPDATE race
  SET checked_in_percentage = (
